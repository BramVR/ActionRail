...
The latest Edit Mode panel polish makes the compact panel draggable so it can
be moved away from rails underneath it. The newest Edit Mode lock affordance
polish replaces visible `Locked` frame text with a minimal Lucide-inspired
lock/unlock icon in each frame's top-right corner, keeps the frame icon
clickable for toggling movement lock state, and uses an icon-only selected-frame
lock button in the compact panel. The latest Edit Mode color polish follows
...
populated slots. The latest README showcase refresh now uses unique hotkey
labels and workflow-specific "spell" slots: a long modeling primitive/action
bar, a short selection context row above it, and side bars for transform,
keying, cleanup, and viewport toggles, so only the current rotate tool reads as
active. The latest slot-layering polish groups contiguous one-row and
...
- Edit Mode shell with layout-map overlay, grid controls, Snap to Grid, Sticky
  ...
  unlocked rails, clickable icon-only lock/unlock controls on selected frames
  and the compact panel,
...
- The May 12 Edit Mode lock affordance polish replaces visible `Locked` frame
  ...
  icon-only selected-frame button with tooltip/accessibility text.
...
- Latest full Maya smoke baseline:
  ...
  scripts.
- Latest focused Edit Mode lock-icon validation:
  `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_edit_mode.py tests\\test_project_map.py`
  -> 44 passed.
- Latest focused Edit Mode lock-icon Ruff:
  `.\\.venv\\Scripts\\python.exe -m ruff check scripts\\actionrail\\edit_mode.py scripts\\actionrail\\project.py tests\\test_edit_mode.py tests\\maya_smoke\\actionrail_edit_mode_smoke.py`
  -> all checks passed.
- Latest Edit Mode Maya smoke after lock-icon polish:
  `.\\scripts\\maya-smoke.ps1 -Script actionrail_edit_mode_smoke.py -Timeout 240`
  -> passed; verified the icon-only compact-panel lock control, clickable
  frame-corner lock/unlock icon, locked-frame nudge blocking, layout/collapse
  persistence, Normal Mode slot drag checks, and refreshed
  `.gg-maya-sessiond/screenshots/actionrail_edit_mode_layout_map.png`.
- Latest README screenshot generation after lock-icon polish:
  `.\\scripts\\maya-smoke.ps1 -Script actionrail_readme_screenshots.py -Timeout 300`
  -> passed; refreshed `docs/assets/actionrail_readme_edit_mode.png` with the
  icon-only frame and panel lock controls. The paired README Maya icon showcase
  was regenerated by the same script.
- Latest docs/project-map checks after lock-icon polish:
...
- Latest README screenshot generation:
  ...
  showcase layout with a long modeling primitive/action bar, a short selection
  context bar above it, transform/keying and cleanup side bars, unique visible
  hotkey labels, and one active rotate-tool slot, using 2x widget capture,
  SVG-backed Maya resources where available, and Maya's current viewport
  background rather than a forced screenshot color.
- Latest focused README screenshot validation:
  `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_action_book.py tests\\test_quick_create.py tests\\test_widgets.py`
  -> 107 passed.
- Latest README screenshot Ruff:
  `.\\.venv\\Scripts\\python.exe -m ruff check tests\\maya_smoke\\actionrail_readme_screenshots.py`
  -> all checks passed.
- Full pytest:
  `.\\.venv\\Scripts\\python.exe -m pytest`
  -> 501 passed.
