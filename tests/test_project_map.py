...
def test_about_returns_json_safe_agent_map() -> None:
    ...
    assert any(
        module["path"] == "scripts/actionrail/action_book_ui.py"
        for module in project["modules"]
    )
...
def test_module_cli_prints_human_project_map(capsys) -> None:
    ...
    assert (
        "Status: Phase 2 step 2.6 collapsible edge-tab handle placement and "
        "publish polish first pass Maya-smoke verified"
    ) in output
    assert (
        "Next: Review and harden the Phase 2 step 2.7 dense overlay foundation "
